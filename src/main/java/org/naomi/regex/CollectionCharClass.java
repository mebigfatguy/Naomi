...
/**
 *
 * An instance of CollectionCharClass is a {@link Pattern} which, on each repetition, matches any single character in the {@link Collection} of
 * {@link Character}s used to instantiate the CollectionCharClass. Any duplicates in the {@link Collection} are ignored. If the collection contains more than a
 * few thousand characters, performance may be adversely impacted. The internal "Not" flag in each instance can be set to make the class instead match all
 * characters <i>except</i> those in the pre-defined set denoted by its instantiating Collection; this has the effect of matching the complement of the
